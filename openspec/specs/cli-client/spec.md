...
The `client` command SHALL accept a `--token` persistent flag. If not provided, it SHALL fall back to the `DASHCAP_API_TOKEN` environment variable. If neither is set, requests are sent without an Authorization header.
